...
export const Route = createFileRoute("/customers")({
  component: () => (
    <AuthGuard>
      <AppLayout>
        <PartiesPage table="customers" title="Customers" />
      </AppLayout>
    </AuthGuard>
  ),
});
                ...
                return (
                  <TableRow key={p.id}>
                    ...
                    <TableCell className="whitespace-nowrap">
                      {formatNPR(p.opening_balance)}
                    </TableCell>
                    <TableCell
                      className={cn(
                        "whitespace-nowrap font-semibold",
                        bal > 0 ? "text-destructive" : bal < 0 ? "text-green-600" : "",
                      )}
                    >
                    ...
                    <TableCell className="text-right whitespace-nowrap">
                      <Button
                        size="icon"
                        variant="ghost"
                        onClick={() => navigate({ to: "/ledger/" + p.id })}
    ...
    <DialogContent>
      ...
      <form onSubmit={submit} className="grid gap-3">
        ...
        <div className="grid gap-3 md:grid-cols-2">
          ...
          <div>
            <Label>PAN</Label>
            <Input value={f.pan} onChange={(e) => setF({ ...f, pan: e.target.value })} />
          </div>
        </div>
        <div>
          <Label>Address</Label>
