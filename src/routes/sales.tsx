...
export const Route = createFileRoute("/sales")({
  component: () => <AuthGuard><AppLayout><SalesPage /></AppLayout></AuthGuard>,
});

interface Sale {
  id: string; invoice_no: string; sale_date: string; customer_id: string | null;
...
function SalesPage() {
  ...
  async function load() {
    const [{ data: s }, { data: c }, { data: p }] = await Promise.all([
      ...
      supabase.from("products").select("id,name,sku,metal,purity,weight_gram,making_charge,stock_qty").order("name"),
    ]);
    setSales((s ?? []) as Sale[]);
  ...
  async function reprint(saleId: string) {
    ...
    const data: BillData = {
      ...
      subtotal: Number(sale.subtotal), making_total: Number(sale.making_total),
      discount: Number(sale.discount), vat_rate: Number(sale.vat_rate),
      vat_amount: Number(sale.vat_amount), total: Number(sale.total),
      paid: Number(sale.paid), due: Number(sale.due),
      payment_mode: sale.payment_mode, notes: sale.notes,
    };
    downloadBill(data);
  }
          ...
          <TableBody>
            {sales.map((s) => (
              <TableRow key={s.id}>
                ...
                <TableCell>{customers.find((c) => c.id === s.customer_id)?.name ?? "Walk-in"}</TableCell>
            ...
            {!sales.length && <TableRow><TableCell colSpan={8} className="text-center text-muted-foreground py-8">No sales yet</TableCell></TableRow>}
          </TableBody>
        </Table>
      </CardContent></Card>
...
function NewSaleDialog({ customers, products, onDone }: { customers: Customer[]; products: Product[]; onDone: () => void }) {
  ...
  async function submit() {
    ...
    try {
      ...
      const { data: sale, error } = await supabase.from("sales").insert({
        ...
        sale_date: date,
        subtotal: totals.subtotal, making_total: totals.making,
        discount, vat_rate: vatRate, vat_amount: totals.vat,
        total: totals.total, paid, due: totals.due,
        payment_mode: paymentMode, notes,
      }).select("*").single();
      if (error) throw error;

      const { error: e2 } = await supabase.from("sale_items").insert(
        itemsPayload.map((it) => ({ ...it, sale_id: sale.id }))
      );
      if (e2) throw e2;
      ...
      for (const l of lines) {
        ...
        if (!prod) continue;
        await supabase.from("products").update({ stock_qty: Math.max(0, prod.stock_qty - l.qty) }).eq("id", l.product_id);
        await supabase.from("stock_movements").insert({
          product_id: l.product_id, type: "out", qty: l.qty, weight_gram: l.weight_gram * l.qty,
          ref_table: "sales", ref_id: sale.id, note: `Sale ${sale.invoice_no}`,
        });
      }
      // Credit ledger
      if (customerId !== "walk-in") {
        await supabase.from("credits").insert({
          party_type: "customer", party_id: customerId, entry_date: date,
          ref_table: "sales", ref_id: sale.id,
          debit: totals.total, credit: paid, note: `Sale ${sale.invoice_no}`,
        });
      }
      // Cashbook
      if (paid > 0) {
        await supabase.from("cashbook").insert({
          entry_date: date, direction: "in", category: "sale", amount: paid,
          party_type: customerId === "walk-in" ? null : "customer",
          party_id: customerId === "walk-in" ? null : customerId,
          ref_table: "sales", ref_id: sale.id, payment_mode: paymentMode,
  ...
  return (
    <DialogContent className="max-w-7xl w-[96vw] max-h-[90vh] overflow-y-auto">
                ...
                <TableRow key={i}>
                  <TableCell className="p-1.5">
                    <Select value={l.product_id ?? "_"} onValueChange={(v) => pickProduct(i, v)}>
                      <SelectTrigger className="w-full h-8 px-2 text-xs"><SelectValue placeholder="Pick" /></SelectTrigger>
                      <SelectContent>
                  ...
                  <TableCell className="p-1.5"><Input className="w-full h-8 px-2 text-xs font-medium" placeholder="Item description" value={l.description} onChange={(e) => updateLine(i, { description: e.target.value })} /></TableCell>
                  <TableCell className="p-1.5">
                    <Select value={l.metal} onValueChange={(v) => updateLine(i, { metal: v as LineItem["metal"] })}>
                      <SelectTrigger className="w-full h-8 px-2 text-xs"><SelectValue /></SelectTrigger>
                      <SelectContent>
                  ...
                  <TableCell className="p-1 px-0 text-center"><Button size="icon" variant="ghost" className="size-7 text-muted-foreground hover:text-destructive" onClick={() => setLines((ls) => ls.filter((_, k) => k !== i))}><Trash2 className="size-3.5" /></Button></TableCell>
                </TableRow>
              );
            })}
          </TableBody>
      ...
      <Button variant="outline" size="sm" className="mt-1" onClick={() => setLines((ls) => [...ls, blankLine()])}><Plus className="size-4 mr-1" />Add line</Button>
