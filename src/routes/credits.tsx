...
export const Route = createFileRoute("/credits")({
  component: () => <AuthGuard><AppLayout><CreditsPage /></AppLayout></AuthGuard>,
});

interface Party { id: string; name: string; opening_balance: number; phone: string | null }
interface Entry { id: string; party_type: "customer" | "supplier"; party_id: string; entry_date: string; debit: number; credit: number; note: string | null; ref_table: string | null }
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between flex-wrap gap-2">
        ...
        <Dialog open={open} onOpenChange={setOpen}>
          <DialogTrigger asChild><Button><Plus className="size-4 mr-2" />Receive / Pay</Button></DialogTrigger>
          <PaymentForm customers={customers} suppliers={suppliers} onDone={() => { setOpen(false); load(); }} />
        </Dialog>
      </div>
      ...
      <Tabs defaultValue="customers">
        ...
        <TabsContent value="customers"><PartyList type="customer" parties={customers} balanceFor={balanceFor} /></TabsContent>
        <TabsContent value="suppliers"><PartyList type="supplier" parties={suppliers} balanceFor={balanceFor} /></TabsContent>
      </Tabs>
    </div>
  );
}

function PartyList({ type, parties, balanceFor }: { type: "customer" | "supplier"; parties: Party[]; balanceFor: (t: "customer" | "supplier", p: Party) => number }) {
  return (
          ...
          {parties.map((p) => {
            ...
            return (
              <TableRow key={p.id}>
                ...
                <TableCell className={bal > 0 ? "text-destructive font-medium" : "text-green-600"}>
                  {formatNPR(bal)}
                </TableCell>
              </TableRow>
            );
          })}
          {!parties.length && <TableRow><TableCell colSpan={3} className="text-center py-8 text-muted-foreground">None</TableCell></TableRow>}
        </TableBody>
      </Table>
    </CardContent></Card>
  );
...
function PaymentForm({ customers, suppliers, onDone }: { customers: Party[]; suppliers: Party[]; onDone: () => void }) {
  const [f, setF] = useState({
    party_type: "customer" as "customer" | "supplier", party_id: "",
    entry_date: todayISO(), amount: 0, payment_mode: "cash", note: "",
  });
  ...
  async function submit(e: React.FormEvent) {
    ...
    const { error } = await supabase.from("credits").insert({
      party_type: f.party_type, party_id: f.party_id, entry_date: f.entry_date,
      debit, credit, note: f.note || (f.party_type === "customer" ? "Payment received" : "Payment made"),
    });
    if (error) return toast.error(error.message);
    await supabase.from("cashbook").insert({
      entry_date: f.entry_date, direction: f.party_type === "customer" ? "in" : "out",
      category: f.party_type === "customer" ? "customer-payment" : "supplier-payment",
      amount: f.amount, party_type: f.party_type, party_id: f.party_id,
      payment_mode: f.payment_mode, note: f.note,
    });
    toast.success("Saved"); onDone();
  }
  return (
    <DialogContent>
      <DialogHeader><DialogTitle>Receive / Pay</DialogTitle></DialogHeader>
      <form onSubmit={submit} className="grid gap-3">
        <div className="grid grid-cols-2 gap-3">
          <div><Label>Type</Label>
            <Select value={f.party_type} onValueChange={(v) => setF({ ...f, party_type: v as "customer" | "supplier", party_id: "" })}>
        ...
        <div className="grid grid-cols-2 gap-3">
          <div><Label>Amount (Rs)</Label><Input type="number" step="0.01" value={f.amount} onChange={(e) => setF({ ...f, amount: Number(e.target.value) })} /></div>
          <div><Label>Mode</Label>
            <Select value={f.payment_mode} onValueChange={(v) => setF({ ...f, payment_mode: v })}>
              ...
              <SelectContent>
                ...
                <SelectItem value="esewa">eSewa</SelectItem><SelectItem value="khalti">Khalti</SelectItem>
              </SelectContent>
            </Select>
          </div>
        </div>
