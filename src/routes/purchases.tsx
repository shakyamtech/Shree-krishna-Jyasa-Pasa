            ...
            <TableBody>
              {items.map((p) => (
                <TableRow key={p.id}>
                  ...
                  <TableCell className={Number(p.due) > 0 ? "text-destructive font-medium" : ""}>
                    {formatNPR(p.due)}
                  </TableCell>
...
function NewPurchaseDialog({
  ...
  const totals = useMemo(() => {
    const subtotal = lines.reduce(
      (s, l) => s + l.qty * (l.weight_gram * l.rate_per_gram + l.making_charge),
      0,
    );
  ...
  async function submit() {
    ...
    try {
      ...
      for (const l of lines) {
        ...
        await supabase
          .from("products")
          .update({ stock_qty: prod.stock_qty + l.qty })
          .eq("id", l.product_id);
            ...
            <Dialog open={newSupOpen} onOpenChange={setNewSupOpen}>
              ...
              <DialogContent className="max-w-md">
                <DialogHeader>
                  ...
                  <DialogDescription>
                    Register a new supplier instantly during purchase intake
                  </DialogDescription>
                </DialogHeader>
                <div className="space-y-3 pt-2">
                  ...
                  <Button className="w-full mt-2" onClick={createSupplierInline}>
                    Save & select
                  </Button>
                </div>
              </DialogContent>
              ...
              return (
                <TableRow key={i}>
                  ...
                  <TableCell>
                    <Select
                      value={l.metal}
                      onValueChange={(v) => update(i, { metal: v as PLine["metal"] })}
                    >
                  ...
                  <TableCell>
                    <Input
                      ...
                      step="0.001"
                      value={l.weight_gram}
                      onChange={(e) => update(i, { weight_gram: Number(e.target.value) })}
                    />
                  </TableCell>
                  <TableCell>
                    <Input
                      ...
                      onChange={(e) => update(i, { rate_per_gram: Number(e.target.value) })}
                    />
                  </TableCell>
                  <TableCell>
                    <Input
                      ...
                      onChange={(e) => update(i, { making_charge: Number(e.target.value) })}
                    />
                  </TableCell>
      ...
      <div className="grid md:grid-cols-2 gap-3 mt-2">
        ...
        <div className="space-y-2 text-sm">
          ...
          <div className="flex justify-between font-bold">
            <span>Due</span>
            <span>{formatNPR(totals.due)}</span>
          </div>
        </div>
      </div>
