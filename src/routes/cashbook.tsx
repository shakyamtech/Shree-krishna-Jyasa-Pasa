...
export const Route = createFileRoute("/cashbook")({
  component: () => <AuthGuard><AppLayout><CashbookPage /></AppLayout></AuthGuard>,
});

interface Entry {
  id: string; entry_date: string; direction: "in" | "out"; category: string;
  amount: number; payment_mode: string; note: string | null;
}
...
function CashbookPage() {
  ...
  async function load() {
    let q = supabase.from("cashbook").select("*").order("entry_date", { ascending: false }).limit(500);
  ...
  const totals = useMemo(() => {
    let inn = 0, out = 0;
    for (const e of items) (e.direction === "in" ? (inn += Number(e.amount)) : (out += Number(e.amount)));
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between flex-wrap gap-2">
        ...
        <Dialog open={open} onOpenChange={setOpen}>
          <DialogTrigger asChild><Button><Plus className="size-4 mr-2" />New entry</Button></DialogTrigger>
          <EntryForm onDone={() => { setOpen(false); load(); }} />
        </Dialog>
      </div>
      ...
      <Card><CardContent className="p-0 overflow-x-auto">
        <Table>
          <TableHeader><TableRow>
            ...
            <TableHead>Mode</TableHead><TableHead>Amount</TableHead><TableHead>Note</TableHead>
          </TableRow></TableHeader>
          <TableBody>
            {items.map((e) => (
              <TableRow key={e.id}>
                <TableCell>{e.entry_date}</TableCell>
                <TableCell><Badge variant={e.direction === "in" ? "default" : "destructive"}>{e.direction === "in" ? "IN" : "OUT"}</Badge></TableCell>
                <TableCell className="capitalize">{e.category}</TableCell>
                <TableCell className="capitalize">{e.payment_mode}</TableCell>
                <TableCell className={"font-medium " + (e.direction === "in" ? "text-green-600" : "text-destructive")}>{formatNPR(e.amount)}</TableCell>
                <TableCell className="max-w-xs truncate">{e.note ?? "—"}</TableCell>
              </TableRow>
            ))}
            {!items.length && <TableRow><TableCell colSpan={6} className="text-center py-8 text-muted-foreground">No entries</TableCell></TableRow>}
          </TableBody>
        </Table>
      </CardContent></Card>
    </div>
...
function EntryForm({ onDone }: { onDone: () => void }) {
  const [f, setF] = useState({
    entry_date: todayISO(), direction: "in" as "in" | "out", category: "other",
    amount: 0, payment_mode: "cash", note: "",
  });
  ...
  return (
    <DialogContent>
      <DialogHeader><DialogTitle>Cashbook entry</DialogTitle></DialogHeader>
      <form onSubmit={submit} className="grid gap-3">
        <div className="grid grid-cols-2 gap-3">
          ...
          <div><Label>Type</Label>
            <Select value={f.direction} onValueChange={(v) => setF({ ...f, direction: v as "in" | "out" })}>
