...
async function getNepaliImage(text: string, fontSize: number, isBold: boolean): Promise<{data: string, w: number, h: number} | null> {
  try {
...
async function writeText(doc: jsPDF, text: string, x: number, y: number, options: { align?: "left" | "center" | "right", bold?: boolean, size?: number } = {}) {
...
export async function generateBillPDF(d: BillData): Promise<jsPDF> {
  ...
  await writeText(doc, `Invoice #: ${d.invoice_no}`, W - 40, y, { align: "right", size: 10 }); y += 14;
  await writeText(doc, `Date: ${d.sale_date}`, W - 40, y, { align: "right", size: 10 }); y += 14;
  await writeText(doc, `मिति: ${toBS(d.sale_date)}`, W - 40, y, { align: "right", size: 10 });

  if (d.customer) {
    await writeText(doc, "Bill To:", 40, y, { bold: true, size: 10 }); y += 12;
    await writeText(doc, d.customer.name, 40, y, { size: 10 }); y += 12;
    if (d.customer.phone) { await writeText(doc, "Phone: " + d.customer.phone, 40, y, { size: 10 }); y += 12; }
    if (d.customer.address) { 
      await writeText(doc, d.customer.address, 40, y, { size: 10 }); 
      y += 12; 
    }
    if (d.customer.pan) { await writeText(doc, "PAN: " + d.customer.pan, 40, y, { size: 10 }); y += 12; }
  } else {
    await writeText(doc, "Walk-in customer", 40, y, { size: 10 }); y += 12;
  }
  y += 6;
  ...
  autoTable(doc, {
    ...
    didDrawCell: (data) => {
      ...
      if (img && data.column.index !== 0) { // Don't replace the # column
        doc.addImage(img.data, "PNG", data.cell.x + 4, data.cell.y + (data.cell.height / 2) - (img.h / 2), img.w, img.h);
      }
    }
  });
